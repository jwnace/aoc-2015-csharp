namespace aoc_2015_csharp.Day21;
...
public static class Day21
{
    ...
    {
        var shop = File.ReadAllText("Day21/day21_shop.txt");

        var sections = shop.Split($"{Environment.NewLine}{Environment.NewLine}");

        var weapons = sections[0].Split(Environment.NewLine)
            .Skip(1)
            .Select(x => x.Split(null).Where(y => !string.IsNullOrWhiteSpace(y)).ToArray())
    ...
    {
        var input = File.ReadAllLines("Day21/day21_input.txt")
