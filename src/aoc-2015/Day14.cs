...
public static class Day14
{
    ...
    public static int Part1()
    {
        ...
        var reindeers = input.Select(x => x.Split(" "))
            ...
            .Select(x => new { Name = x.Name, Distance = x.Fly(2503) })
            .ToList();
    ...
    public static int Part2()
    {
        ...
        {
            var sim = reindeers.Select(x => new { Name = x.Name, Distance = x.Fly(i) }).ToList();
            var max = sim.Max(x => x.Distance);
            var leaders = sim.Where(x => x.Distance == max).ToList();

            foreach (var leader in leaders)
            {
                scores[leader.Name] = scores.ContainsKey(leader.Name) ? scores[leader.Name] + 1 : 1;
            }
        }
